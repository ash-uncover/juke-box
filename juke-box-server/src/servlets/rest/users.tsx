...
export const getUser = function(req, res, next) {
    LOGGER.debug('GET ' + req.url)
    try {
        defaultGet(SCHEMAS.USERS, req, res, next)
    } catch (error) {
        res.send(500, error)
    }
}
...
export const deleteUser = function(req, res, next) {
    ...
    try {
        defaultDelete(SCHEMAS.USERS, req, res, next)
    } catch (error) {
        res.send(500, error)
    }
}
export const getUserMemberships = function(req, res, next) {
    LOGGER.debug('GET ' + req.url)
    try {
        defaultGetDeep(SCHEMAS.MEMBERSHIPS, req, res, next)
    } catch (error) {
        res.send(500, error)
    }
}
export const getUserEvents = function(req, res, next) {
    LOGGER.debug('GET ' + req.url)
    try {
        defaultGetDeep(SCHEMAS.EVENTS, req, res, next)
    } catch (error) {
        res.send(500, error)
    }
}